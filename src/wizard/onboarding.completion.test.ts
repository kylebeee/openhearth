...
describe("setupOnboardingShellCompletion", () => {
  it("QuickStart: installs without prompting", async () => {
    ...
    const deps = {
      resolveCliName: () => "openhearth",
      checkShellCompletionStatus: vi.fn(async () => ({
        shell: "zsh",
        profileInstalled: false,
        cacheExists: false,
        cachePath: "/tmp/openhearth.zsh",
    ...
    expect(deps.ensureCompletionCacheExists).toHaveBeenCalledWith("openhearth");
    expect(deps.installCompletion).toHaveBeenCalledWith("zsh", true, "openhearth");
