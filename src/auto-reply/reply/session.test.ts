...
describe("initSessionState RawBody", () => {
  ...
  it("uses the default per-agent sessions store when config store is unset", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "openhearth-session-store-default-"));
    const stateDir = path.join(root, ".openhearth");
    ...
    vi.stubEnv("OPENHEARTH_STATE_DIR", stateDir);
    try {
