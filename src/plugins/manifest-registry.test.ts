...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `openhearth-manifest-registry-${randomUUID()}`);
...
function writeManifest(dir: string, manifest: Record<string, unknown>) {
  fs.writeFileSync(path.join(dir, "openhearth.plugin.json"), JSON.stringify(manifest), "utf-8");
}
