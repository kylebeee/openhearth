...
describe("daemon-cli coverage", () => {
  const originalEnv = {
    ...
    OPENHEARTH_CONFIG_PATH: process.env.OPENHEARTH_CONFIG_PATH,
    OPENHEARTH_GATEWAY_PORT: process.env.OPENHEARTH_GATEWAY_PORT,
    OPENHEARTH_PROFILE: process.env.OPENHEARTH_PROFILE,
  };

  beforeEach(() => {
    process.env.OPENHEARTH_STATE_DIR = "/tmp/openhearth-cli-state";
    process.env.OPENHEARTH_CONFIG_PATH = "/tmp/openhearth-cli-state/openhearth.json";
    delete process.env.OPENHEARTH_GATEWAY_PORT;
    delete process.env.OPENHEARTH_PROFILE;
    serviceReadCommand.mockResolvedValue(null);
  });

  afterEach(() => {
    if (originalEnv.OPENHEARTH_STATE_DIR !== undefined) {
      process.env.OPENHEARTH_STATE_DIR = originalEnv.OPENHEARTH_STATE_DIR;
    } else {
      delete process.env.OPENHEARTH_STATE_DIR;
    }

    if (originalEnv.OPENHEARTH_CONFIG_PATH !== undefined) {
      process.env.OPENHEARTH_CONFIG_PATH = originalEnv.OPENHEARTH_CONFIG_PATH;
    } else {
      delete process.env.OPENHEARTH_CONFIG_PATH;
    }

    if (originalEnv.OPENHEARTH_GATEWAY_PORT !== undefined) {
      process.env.OPENHEARTH_GATEWAY_PORT = originalEnv.OPENHEARTH_GATEWAY_PORT;
    } else {
      delete process.env.OPENHEARTH_GATEWAY_PORT;
    }

    if (originalEnv.OPENHEARTH_PROFILE !== undefined) {
      process.env.OPENHEARTH_PROFILE = originalEnv.OPENHEARTH_PROFILE;
    } else {
      delete process.env.OPENHEARTH_PROFILE;
    }
  });
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        OPENHEARTH_PROFILE: "dev",
        OPENHEARTH_STATE_DIR: "/tmp/openhearth-daemon-state",
        OPENHEARTH_CONFIG_PATH: "/tmp/openhearth-daemon-state/openhearth.json",
        OPENHEARTH_GATEWAY_PORT: "19001",
      },
