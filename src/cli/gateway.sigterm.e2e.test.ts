...
describe("gateway SIGTERM", () => {
  ...
  it("exits 0 on SIGTERM", { timeout: 180_000 }, async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "openhearth-gateway-test-"));
    const out: string[] = [];
    const err: string[] = [];
    ...
    const env = {
      ...process.env,
      OPENHEARTH_NO_RESPAWN: "1",
      OPENHEARTH_STATE_DIR: stateDir,
      OPENHEARTH_SKIP_CHANNELS: "1",
      OPENHEARTH_SKIP_GMAIL_WATCHER: "1",
      OPENHEARTH_SKIP_CRON: "1",
      OPENHEARTH_SKIP_BROWSER_CONTROL_SERVER: "1",
      OPENHEARTH_SKIP_CANVAS_HOST: "1",
    };
    const bootstrapPath = path.join(stateDir, "openhearth-entry-bootstrap.mjs");
