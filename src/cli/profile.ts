...
export function applyCliProfileEnv(params: {
  ...
  env.OPENHEARTH_PROFILE = profile;

  const stateDir =
    env.OPENHEARTH_STATE_DIR?.trim() || resolveProfileStateDir(profile, env, homedir);
  if (!env.OPENHEARTH_STATE_DIR?.trim()) {
    env.OPENHEARTH_STATE_DIR = stateDir;
  }

  if (!env.OPENHEARTH_CONFIG_PATH?.trim()) {
    env.OPENHEARTH_CONFIG_PATH = path.join(stateDir, "openhearth.json");
  }

  if (profile === "dev" && !env.OPENHEARTH_GATEWAY_PORT?.trim()) {
    env.OPENHEARTH_GATEWAY_PORT = "19001";
  }
}
