...
${formatHelpExamples([
  ['openhearth agent --to +15555550123 --message "status update"', "Start a new session."],
  ['openhearth agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'openhearth agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'openhearth agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['openhearth agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'openhearth agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.openhearth.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.openhearth.ai/cli/agents")}\n`,
    );
...
${formatHelpExamples([
  [
    'openhearth agents set-identity --agent main --name "OpenHearth" --emoji "🦞"',
    "Set name + emoji.",
  ],
  [
  ...
  [
    "openhearth agents set-identity --identity-file ~/.openhearth/workspace/IDENTITY.md --agent main",
