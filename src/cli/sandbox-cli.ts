...
const SANDBOX_EXAMPLES = {
  main: [
    ["openhearth sandbox list", "List all sandbox containers."],
    ["openhearth sandbox list --browser", "List only browser containers."],
    ["openhearth sandbox recreate --all", "Recreate all containers."],
    ["openhearth sandbox recreate --session main", "Recreate a specific session."],
    ["openhearth sandbox recreate --agent mybot", "Recreate agent containers."],
    ["openhearth sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["openhearth sandbox list", "List all sandbox containers."],
    ["openhearth sandbox list --browser", "List only browser containers."],
  ...
  recreate: [
    ...
    ["openhearth sandbox recreate --session main", "Recreate a specific session."],
    [
      "openhearth sandbox recreate --agent mybot",
      "Recreate a specific agent (includes sub-agents).",
    ],
    ["openhearth sandbox recreate --browser --all", "Recreate only browser containers."],
    ["openhearth sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["openhearth sandbox explain", "Show effective sandbox config."],
    ["openhearth sandbox explain --session agent:main:main", "Explain a specific session."],
    ["openhearth sandbox explain --agent work", "Explain an agent sandbox."],
    ["openhearth sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "docs.openhearth.ai/cli/sandbox")}\n`,
    )
    .action(() => {
