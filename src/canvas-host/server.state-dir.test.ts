...
describe("canvas host state dir defaults", () => {
  ...
  it("uses OPENHEARTH_STATE_DIR for the default canvas root", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "openhearth-canvas-state-"));
    ...
    try {
      ...
      expect(indexContents).toContain("OpenHearth Canvas");
    } finally {
      await handler.close();
