...
describe("resolveSessionTranscriptCandidates safety", () => {
  test("keeps cross-agent absolute sessionFile when storePath agent context differs", () => {
    ...
    const sessionFile = "/tmp/openhearth/agents/ops/sessions/sess-safe.jsonl";
...
describe("archiveSessionTranscripts", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "openhearth-archive-test-"));
    storePath = path.join(tmpDir, "sessions.json");
    vi.stubEnv("OPENHEARTH_HOME", tmpDir);
  });
