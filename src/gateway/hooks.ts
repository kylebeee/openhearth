...
export function resolveHooksConfig(cfg: OpenHearthConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: OpenHearthConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-openhearth-token"] === "string"
      ? req.headers["x-openhearth-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
