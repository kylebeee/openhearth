...
beforeAll(async () => {
  ...
  prevGatewayToken = process.env.OPENHEARTH_GATEWAY_TOKEN;
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.OPENHEARTH_GATEWAY_PORT = String(gatewayPort);
  process.env.OPENHEARTH_GATEWAY_TOKEN = gatewayToken;
...
afterAll(async () => {
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.OPENHEARTH_GATEWAY_TOKEN = prevGatewayToken;
  }
});
...
describe("sessions_send gateway loopback", () => {
  it("returns reply when lifecycle ends before agent.wait", async () => {
    ...
    const tool = createOpenHearthTools().find((candidate) => candidate.name === "sessions_send");
    if (!tool) {
