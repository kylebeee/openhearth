...
describe("gateway e2e", () => {
  it(
    ...
    async () => {
      const prev = {
        ...
        skipBrowser: process.env.OPENHEARTH_SKIP_BROWSER_CONTROL_SERVER,
      };

      const { baseUrl: openaiBaseUrl, restore } = installOpenAiResponsesMock();

      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "openhearth-gw-mock-home-"));
      ...
      const workspaceDir = path.join(tempHome, "openhearth");
      await fs.mkdir(workspaceDir, { recursive: true });

      const nonceA = randomUUID();
      const nonceB = randomUUID();
      const toolProbePath = path.join(workspaceDir, `.openhearth-tool-probe.${nonceA}.txt`);
      ...
      try {
        ...
        process.env.OPENHEARTH_SKIP_BROWSER_CONTROL_SERVER = prev.skipBrowser;
      }
    },
  );
  ...
  it("runs wizard over ws and writes auth token config", { timeout: 90_000 }, async () => {
    ...
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "openhearth-wizard-home-"));
    process.env.HOME = tempHome;
    ...
    try {
      ...
      await fs.rm(tempHome, { recursive: true, force: true });
      process.env.HOME = prev.home;
      process.env.OPENHEARTH_STATE_DIR = prev.stateDir;
      process.env.OPENHEARTH_CONFIG_PATH = prev.configPath;
      process.env.OPENHEARTH_GATEWAY_TOKEN = prev.token;
      process.env.OPENHEARTH_SKIP_CHANNELS = prev.skipChannels;
      process.env.OPENHEARTH_SKIP_GMAIL_WATCHER = prev.skipGmail;
