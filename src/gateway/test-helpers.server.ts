...
async function setupGatewayTestHome() {
  ...
  previousSkipChannels = process.env.OPENHEARTH_SKIP_CHANNELS;
  previousSkipProviders = process.env.OPENHEARTH_SKIP_PROVIDERS;
  previousSkipCron = process.env.OPENHEARTH_SKIP_CRON;
  previousMinimalGateway = process.env.OPENHEARTH_TEST_MINIMAL_GATEWAY;
...
function applyGatewaySkipEnv() {
  ...
  process.env.OPENHEARTH_SKIP_PROVIDERS = "1";
  process.env.OPENHEARTH_SKIP_CRON = "1";
  process.env.OPENHEARTH_TEST_MINIMAL_GATEWAY = "1";
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousSkipChannels === undefined) {
      ...
      process.env.OPENHEARTH_SKIP_CHANNELS = previousSkipChannels;
    }
    if (previousSkipProviders === undefined) {
      delete process.env.OPENHEARTH_SKIP_PROVIDERS;
    } else {
      process.env.OPENHEARTH_SKIP_PROVIDERS = previousSkipProviders;
    }
    if (previousSkipCron === undefined) {
      delete process.env.OPENHEARTH_SKIP_CRON;
    } else {
      process.env.OPENHEARTH_SKIP_CRON = previousSkipCron;
    }
    if (previousMinimalGateway === undefined) {
      delete process.env.OPENHEARTH_TEST_MINIMAL_GATEWAY;
    } else {
      process.env.OPENHEARTH_TEST_MINIMAL_GATEWAY = previousMinimalGateway;
    }
  }
