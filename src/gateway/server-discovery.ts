...
export function formatBonjourInstanceName(displayName: string) {
  ...
  if (/openhearth/i.test(trimmed)) {
    return trimmed;
  }
  return `${trimmed} (OpenHearth)`;
}

export function resolveBonjourCliPath(opts: ResolveBonjourCliPathOptions = {}): string | undefined {
  const env = opts.env ?? process.env;
  const envPath = env.OPENHEARTH_CLI_PATH?.trim();
  if (envPath) {
    return envPath;
  }
  ...
  const binCli = path.join(cwd, "bin", "openhearth");
  if (isFile(binCli)) {
    return binCli;
  }
...
export async function resolveTailnetDnsHint(opts?: {
  ...
  const env = opts?.env ?? process.env;
  const envRaw = env.OPENHEARTH_TAILNET_DNS?.trim();
