...
import type { OpenHearthConfig, ModelProviderConfig } from "../config/types.js";
...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.OPENHEARTH_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.OPENHEARTH_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.OPENHEARTH_LIVE_GATEWAY_PROVIDERS);
...
function sanitizeAuthConfig(params: {
  cfg: OpenHearthConfig;
  agentDir: string;
}): OpenHearthConfig["auth"] | undefined {
  ...
  let profiles: NonNullable<OpenHearthConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    ...
    agentDir: process.env.OPENHEARTH_AGENT_DIR,
  ...
  const hostAgentDir = resolveOpenHearthAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "openhearth-live-state-"));
  ...
  process.env.OPENHEARTH_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.openhearth-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveOpenHearthAgentDir();
  const sanitizedCfg: OpenHearthConfig = {
    ...params.cfg,
  ...
  try {
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "OpenHearth live tool probe (local, safe): " +
    ...
    process.env.OPENHEARTH_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.OPENHEARTH_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      const rawModels = process.env.OPENHEARTH_LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.openhearth-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
