...
async function withTempConfig(params: { cfg: unknown; run: () => Promise<void> }): Promise<void> {
  const prevConfigPath = process.env.OPENHEARTH_CONFIG_PATH;
  const prevDisableCache = process.env.OPENHEARTH_DISABLE_CONFIG_CACHE;

  const dir = await mkdtemp(path.join(os.tmpdir(), "openhearth-plugin-http-auth-test-"));
  ...
  try {
    ...
    if (prevConfigPath === undefined) {
      ...
      process.env.OPENHEARTH_CONFIG_PATH = prevConfigPath;
    }
    if (prevDisableCache === undefined) {
      delete process.env.OPENHEARTH_DISABLE_CONFIG_CACHE;
    } else {
      process.env.OPENHEARTH_DISABLE_CONFIG_CACHE = prevDisableCache;
    }
