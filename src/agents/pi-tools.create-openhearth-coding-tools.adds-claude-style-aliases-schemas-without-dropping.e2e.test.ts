...
import { __testing, createOpenHearthCodingTools } from "./pi-tools.js";
...
describe("createOpenHearthCodingTools", () => {
  ...
  it("keeps raw core tool schemas union-free", () => {
    const tools = createOpenHearthTools();
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createOpenHearthCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "openhearth-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "openhearth-outside.txt");
