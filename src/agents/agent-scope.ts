...
type AgentEntry = NonNullable<NonNullable<OpenHearthConfig["agents"]>["list"]>[number];
...
function listAgents(cfg: OpenHearthConfig): AgentEntry[] {
...
export function listAgentIds(cfg: OpenHearthConfig): string[] {
...
export function resolveDefaultAgentId(cfg: OpenHearthConfig): string {
  const agents = listAgents(cfg);
  if (agents.length === 0) {
    return DEFAULT_AGENT_ID;
  }
...
function resolveAgentEntry(cfg: OpenHearthConfig, agentId: string): AgentEntry | undefined {
...
export function resolveAgentWorkspaceDir(cfg: OpenHearthConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
...
export function resolveAgentDir(cfg: OpenHearthConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
