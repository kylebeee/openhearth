...
const _MODELS_CONFIG: OpenHearthConfig = {
  models: {
    providers: {
      "custom-proxy": {
...
describe("models-config", () => {
  ...
  it("normalizes gemini 3 ids to preview for google providers", async () => {
    await withTempHome(async () => {
      const { ensureOpenHearthModelsJson } = await import("./models-config.js");
      const { resolveOpenHearthAgentDir } = await import("./agent-paths.js");
