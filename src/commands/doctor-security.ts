...
import type { OpenHearthConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: OpenHearthConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("openhearth security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("openhearth configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("openhearth config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("openhearth doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("openhearth config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
