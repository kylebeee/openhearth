...
const SEARCH_TOOL = "https://docs.openhearth.ai/mcp.SearchOpenHearth";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.openhearth.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('openhearth docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.openhearth.ai/");
      runtime.log(`Search: ${formatCliCommand('openhearth docs "your query"')}`);
    }
    return;
  }
