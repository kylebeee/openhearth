...
export async function requireValidConfig(runtime: RuntimeEnv): Promise<OpenHearthConfig | null> {
  ...
  if (snapshot.exists && !snapshot.valid) {
    ...
    runtime.error(`Fix the config or run ${formatCliCommand("openhearth doctor")}.`);
    runtime.exit(1);
    return null;
  }
