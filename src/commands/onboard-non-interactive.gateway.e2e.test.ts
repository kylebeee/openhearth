...
describe("onboard (non-interactive): gateway and remote auth", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "openhearth-onboard-"));
  ...
  afterAll(async () => {
    ...
    process.env.OPENHEARTH_GATEWAY_PASSWORD = prev.password;
  });
  ...
  it("writes gateway token auth into config and gateway enforces it", async () => {
    ...
    const workspace = path.join(stateDir, "openhearth");
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    process.env.OPENHEARTH_STATE_DIR = stateDir;
    process.env.OPENHEARTH_CONFIG_PATH = path.join(stateDir, "openhearth.json");
