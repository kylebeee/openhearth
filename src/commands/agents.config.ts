...
export function listAgentEntries(cfg: OpenHearthConfig): AgentEntry[] {
...
function resolveAgentName(cfg: OpenHearthConfig, agentId: string) {
...
function resolveAgentModel(cfg: OpenHearthConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: OpenHearthConfig): AgentSummary[] {
