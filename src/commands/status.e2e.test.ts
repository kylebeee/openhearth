...
beforeAll(() => {
  previousProfile = process.env.OPENHEARTH_PROFILE;
  process.env.OPENHEARTH_PROFILE = "isolated";
});
...
afterAll(() => {
  if (previousProfile === undefined) {
    ...
    process.env.OPENHEARTH_PROFILE = previousProfile;
  }
});
...
vi.mock("../infra/openhearth-root.js", () => ({
  resolveOpenHearthPackageRoot: vi.fn().mockResolvedValue("/tmp/openhearth"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/openhearth/pnpm-lock.yaml",
      markerPath: "/tmp/openhearth/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    await statusCommand({}, runtime as never);
    const logs = (runtime.log as vi.Mock).mock.calls.map((c) => String(c[0]));
    expect(logs.some((l) => l.includes("OpenHearth status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("openhearth status --all") ||
          l.includes("openhearth --profile isolated status --all") ||
          l.includes("openhearth status --all") ||
          l.includes("openhearth --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.OPENHEARTH_GATEWAY_TOKEN = "abcd1234";
    try {
