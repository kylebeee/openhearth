  ...
  try {
    if (boxes.length > 0) {
      await page.evaluate((labels) => {
        const existing = document.querySelectorAll("[data-openhearth-labels]");
        existing.forEach((el) => el.remove());

        const root = document.createElement("div");
        root.setAttribute("data-openhearth-labels", "1");
        ...
        for (const label of labels) {
          ...
          box.setAttribute("data-openhearth-labels", "1");
          ...
          tag.setAttribute("data-openhearth-labels", "1");
