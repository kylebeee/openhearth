...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".openhearth-profile-decorated");
}
 ...
 */
export function decorateOpenHearthProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_OPENHEARTH_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_OPENHEARTH_BROWSER_COLOR).toUpperCase();
