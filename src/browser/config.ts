import type { BrowserConfig, BrowserProfileConfig, OpenHearthConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_OPENHEARTH_BROWSER_ENABLED,
...
export type ResolvedBrowserProfile = {
  ...
  cdpIsLoopback: boolean;
  color: string;
  driver: "openhearth" | "extension";
};

function normalizeHexColor(raw: string | undefined) {
  const value = (raw ?? "").trim();
  if (!value) {
    return DEFAULT_OPENHEARTH_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "openhearth" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_OPENHEARTH_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_OPENHEARTH_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an OpenHearth browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for kylebeee/openhearth CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: OpenHearthConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_OPENHEARTH_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "openhearth";
