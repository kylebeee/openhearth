...
vi.mock("./chrome.js", () => ({
  resolveOpenHearthUserDataDir: vi.fn(() => "/tmp/openhearth-test/openhearth/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveOpenHearthUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "openhearth-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveOpenHearthUserDataDir).mockReturnValue(userDataDir);
