...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.OPENHEARTH_GATEWAY_PORT).toBe("18789");
    expect(env.OPENHEARTH_GATEWAY_TOKEN).toBe("secret");
    expect(env.OPENHEARTH_SERVICE_MARKER).toBe("openhearth");
    expect(env.OPENHEARTH_SERVICE_KIND).toBe("gateway");
    expect(typeof env.OPENHEARTH_SERVICE_VERSION).toBe("string");
    expect(env.OPENHEARTH_SYSTEMD_UNIT).toBe("openhearth-gateway.service");
    if (process.platform === "darwin") {
      expect(env.OPENHEARTH_LAUNCHD_LABEL).toBe("ai.openhearth.gateway");
    }
  });

  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", OPENHEARTH_PROFILE: "work" },
      port: 18789,
    });
    expect(env.OPENHEARTH_SYSTEMD_UNIT).toBe("openhearth-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.OPENHEARTH_LAUNCHD_LABEL).toBe("ai.openhearth.work");
    }
  });
});
