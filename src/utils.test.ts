...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    const tmp = await fs.promises.mkdtemp(path.join(os.tmpdir(), "openhearth-test-"));
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    const authDir = fs.mkdtempSync(path.join(os.tmpdir(), "openhearth-auth-"));
  ...
  it("falls back through lidMappingDirs in order", () => {
    const first = fs.mkdtempSync(path.join(os.tmpdir(), "openhearth-lid-a-"));
    const second = fs.mkdtempSync(path.join(os.tmpdir(), "openhearth-lid-b-"));
...
describe("resolveConfigDir", () => {
  it("prefers ~/.openhearth when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "openhearth-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers OPENHEARTH_HOME over HOME", () => {
    vi.stubEnv("OPENHEARTH_HOME", "/srv/openhearth-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/openhearth-home"));
...
describe("shortenHomePath", () => {
  it("uses $OPENHEARTH_HOME prefix when OPENHEARTH_HOME is set", () => {
    vi.stubEnv("OPENHEARTH_HOME", "/srv/openhearth-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomePath(`${path.resolve("/srv/openhearth-home")}/.kylebeee/openhearth.json`),
    ).toBe("$OPENHEARTH_HOME/.kylebeee/openhearth.json");
...
describe("shortenHomeInString", () => {
  it("uses $OPENHEARTH_HOME replacement when OPENHEARTH_HOME is set", () => {
    vi.stubEnv("OPENHEARTH_HOME", "/srv/openhearth-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(
        `config: ${path.resolve("/srv/openhearth-home")}/.kylebeee/openhearth.json`,
      ),
    ).toBe("config: $OPENHEARTH_HOME/.kylebeee/openhearth.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/openhearth")).toBe(path.resolve(os.homedir(), "openhearth"));
  });
  ...
  it("prefers OPENHEARTH_HOME for tilde expansion", () => {
    vi.stubEnv("OPENHEARTH_HOME", "/srv/openhearth-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/openhearth")).toBe(
      path.resolve("/srv/openhearth-home", "openhearth"),
    );
