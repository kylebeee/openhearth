...
describe("tailscale helpers", () => {
  const originalForcedBinary = process.env.OPENHEARTH_TEST_TAILSCALE_BINARY;

  beforeEach(() => {
    process.env.OPENHEARTH_TEST_TAILSCALE_BINARY = "tailscale";
  });

  afterEach(() => {
    if (originalForcedBinary === undefined) {
      delete process.env.OPENHEARTH_TEST_TAILSCALE_BINARY;
    } else {
      process.env.OPENHEARTH_TEST_TAILSCALE_BINARY = originalForcedBinary;
    }
    vi.restoreAllMocks();
  });
