...
export function resolvePreferredOpenHearthTmpDir(
  ...
  const fallback = (): string => {
    const base = tmpdir();
    const suffix = uid === undefined ? "openhearth" : `openhearth-${uid}`;
    return path.join(base, suffix);
  };

  try {
    const preferred = lstatSync(POSIX_OPENHEARTH_TMP_DIR);
  ...
  try {
    ...
    mkdirSync(POSIX_OPENHEARTH_TMP_DIR, { recursive: true, mode: 0o700 });
    try {
      const preferred = lstatSync(POSIX_OPENHEARTH_TMP_DIR);
      if (!preferred.isDirectory() || preferred.isSymbolicLink()) {
        return fallback();
