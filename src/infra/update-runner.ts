  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(
        path.join(os.tmpdir(), "openhearth-update-preflight-"),
      );
    ...
    const doctorEntry = path.join(gitRoot, "openhearth.mjs");
    ...
    const doctorStep = await runStep(
      step("openhearth doctor", doctorArgv, gitRoot, { OPENHEARTH_UPDATE_IN_PROGRESS: "1" }),
    );
