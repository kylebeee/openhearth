...
describe("resolveEffectiveHomeDir", () => {
  it("prefers OPENHEARTH_HOME over HOME and USERPROFILE", () => {
    const env = {
    ...
    expect(resolveEffectiveHomeDir(env, () => "/fallback")).toBe(
      path.resolve("/srv/openhearth-home"),
    );
  });
  ...
  it("expands OPENHEARTH_HOME when set to ~", () => {
    const env = {
      OPENHEARTH_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for OPENHEARTH_HOME", () => {
    const result = resolveRequiredHomeDir(
      { OPENHEARTH_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when OPENHEARTH_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ OPENHEARTH_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { OPENHEARTH_HOME: "/srv/openhearth-home" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(`${path.resolve("/srv/openhearth-home")}/x`);
  });
