...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.OPENHEARTH_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*openhearth-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "openhearth";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "openhearth");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "openhearth-gw");
  ...
  records.push(`_openhearth-gw._tcp IN PTR ${instanceLabel}._openhearth-gw._tcp`);
  records.push(`${instanceLabel}._openhearth-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._openhearth-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; openhearth-content-hash: ${contentHash}\n${contentBody}`;
}
