...
export async function getTailscaleBinary(): Promise<string> {
  const forcedBinary = process.env.OPENHEARTH_TEST_TAILSCALE_BINARY?.trim();
  if (forcedBinary) {
...
export async function ensureFunnel(
  ...
  try {
    ...
    runtime.error(
      info(
        `Tip: Funnel is optional for OpenHearth. You can keep running the web gateway without it: \`${formatCliCommand("openhearth gateway")}\``,
      ),
    );
    if (shouldLogVerbose()) {
      const rich = isRich();
