...
describe("gateway bonjour advertiser", () => {
  ...
  it("does not block on advertise and publishes expected txt keys", async () => {
    ...
    process.env.OPENHEARTH_MDNS_HOSTNAME = "test-host";
    process.env.OPENHEARTH_MDNS_HOSTNAME = "test-host";
    ...
    expect(gatewayCall?.[0]?.type).toBe("openhearth-gw");
  ...
  it("normalizes hostnames with domains for service names", async () => {
    ...
    expect(gatewayCall?.[0]?.name).toBe("openhearth (OpenHearth)");
    expect(gatewayCall?.[0]?.domain).toBe("local");
    expect(gatewayCall?.[0]?.hostname).toBe("openhearth");
    expect((gatewayCall?.[0]?.txt as Record<string, string>)?.lanHost).toBe("openhearth.local");
