...
function stubChannelPlugin(params: {
  ...
  label: string;
  resolveAccount: (cfg: OpenHearthConfig) => unknown;
}): ChannelPlugin {
  return {
    id: params.id,
    meta: {
...
describe("security audit", () => {
  ...
  it("flags non-loopback bind without auth as critical", async () => {
    ...
    const prevPassword = process.env.OPENHEARTH_GATEWAY_PASSWORD;
  ...
  it("warns when Telegram allowFrom entries are non-numeric (legacy @username configs)", async () => {
    const prevStateDir = process.env.OPENHEARTH_STATE_DIR;
    const tmp = await fs.mkdtemp(
      path.join(os.tmpdir(), "openhearth-security-audit-telegram-invalid-allowfrom-"),
    );
    process.env.OPENHEARTH_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
