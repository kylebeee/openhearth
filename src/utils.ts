...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".openhearth");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.OPENHEARTH_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$OPENHEARTH_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via OPENHEARTH_STATE_DIR.
