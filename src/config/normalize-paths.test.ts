  ...
  it("expands tilde for path-ish keys only", async () => {
    await withTempHome(async (home) => {
      const cfg = normalizeConfigPaths({
        ...
        logging: { file: "~/.openhearth/logs/openhearth.log" },
        hooks: {
          path: "~/.openhearth/hooks.json5",
        ...
        channels: {
          telegram: {
            accounts: {
              personal: {
                tokenFile: "~/.openhearth/telegram.token",
              },
            },
          },
          imessage: {
      ...
      expect(cfg.logging?.file).toBe(path.join(home, ".openhearth", "logs", "openhearth.log"));
      expect(cfg.hooks?.path).toBe(path.join(home, ".openhearth", "hooks.json5"));
      ...
      expect(cfg.channels?.telegram?.accounts?.personal?.tokenFile).toBe(
        path.join(home, ".openhearth", "telegram.token"),
      );
