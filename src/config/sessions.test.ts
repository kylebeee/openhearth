...
describe("sessions", () => {
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-openhearth",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-openhearth#general");
  });
  ...
  it("derives session transcripts dir from OPENHEARTH_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { OPENHEARTH_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("includes topic ids in session transcript filenames", () => {
    ...
    process.env.OPENHEARTH_STATE_DIR = "/custom/state";
    try {
