...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as OpenHearthConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  async function writeConfigFile(cfg: OpenHearthConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfig =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as OpenHearthConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.OPENHEARTH_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.OPENHEARTH_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.OPENHEARTH_WATCH_SESSION === "string" &&
        deps.env.OPENHEARTH_WATCH_SESSION.trim().length > 0
          ? deps.env.OPENHEARTH_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.OPENHEARTH_WATCH_COMMAND === "string" &&
        deps.env.OPENHEARTH_WATCH_COMMAND.trim().length > 0
          ? deps.env.OPENHEARTH_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
