...
const ETC_OPENHEARTH_DIR = path.join(ROOT_DIR, "etc", "openhearth");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "openhearth.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcOpenHearthPath(...parts: string[]) {
  return path.join(ETC_OPENHEARTH_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcOpenHearthPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "openhearth.json"))).toEqual({
      shared: true,
    });
  });
});
