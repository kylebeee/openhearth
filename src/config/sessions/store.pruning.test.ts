...
// Mock loadConfig so resolveMaintenanceConfig() never reads a real openhearth.json.
...
describe("rotateSessionFile", () => {
  ...
  beforeEach(async () => {
    testDir = await fs.mkdtemp(path.join(os.tmpdir(), "openhearth-rotate-"));
...
describe("Integration: saveSessionStore with pruning", () => {
  ...
  beforeEach(async () => {
    testDir = await fs.mkdtemp(path.join(os.tmpdir(), "openhearth-pruning-integ-"));
    storePath = path.join(testDir, "sessions.json");
    savedCacheTtl = process.env.OPENHEARTH_SESSION_CACHE_TTL_MS;
    process.env.OPENHEARTH_SESSION_CACHE_TTL_MS = "0";
    clearSessionStoreCacheForTest();
  ...
  afterEach(async () => {
    ...
    if (savedCacheTtl === undefined) {
      delete process.env.OPENHEARTH_SESSION_CACHE_TTL_MS;
    } else {
      process.env.OPENHEARTH_SESSION_CACHE_TTL_MS = savedCacheTtl;
    }
  });
