...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["openhearthPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["openhearth", "pollId"]) ??
    readNestedString(value, ["openhearth", "poll", "id"]) ??
    readNestedString(value, ["data", "openhearthPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "openhearth", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          openhearthPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "openhearth poll vote",
          displayText: "Vote recorded",
          value: { openhearthPollId: pollId, pollId },
        },
      },
    ],
  };
