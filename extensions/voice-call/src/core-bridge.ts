...
function resolveOpenHearthRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.OPENHEARTH_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set OPENHEARTH_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveOpenHearthRoot(), "dist", "extensionAPI.js");
