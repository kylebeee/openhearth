...
import { approveDevicePairing, listDevicePairing } from "openhearth/plugin-sdk";
...
function resolveGatewayPort(cfg: OpenHearthPluginApi["config"]): number {
  const envRaw =
    process.env.OPENHEARTH_GATEWAY_PORT?.trim() || process.env.CLAWDBOT_GATEWAY_PORT?.trim();
  if (envRaw) {
...
function resolveScheme(
  cfg: OpenHearthPluginApi["config"],
...
async function resolveTailnetHost(api: OpenHearthPluginApi): Promise<string | null> {
...
function resolveAuth(cfg: OpenHearthPluginApi["config"]): ResolveAuthResult {
...
async function resolveGatewayUrl(api: OpenHearthPluginApi): Promise<ResolveUrlResult> {
