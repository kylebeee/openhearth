import type { OpenHearthPluginApi, OpenHearthPluginService } from "openhearth/plugin-sdk";
...
function normalizeDenyList(cfg: OpenHearthPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: OpenHearthPluginApi["config"]): string[] {
...
export default function register(api: OpenHearthPluginApi) {
  ...
  const timerService: OpenHearthPluginService = {
