...
const liveEnabled = HAS_OPENAI_KEY && process.env.OPENHEARTH_LIVE_TEST === "1";
...
describe("memory plugin e2e", () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "openhearth-memory-test-"));
...
describeLive("memory plugin live tests", () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "openhearth-memory-live-"));
    dbPath = path.join(tmpDir, "lancedb");
  });
