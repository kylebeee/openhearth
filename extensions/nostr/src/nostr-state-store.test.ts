...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.OPENHEARTH_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "openhearth-nostr-"));
  process.env.OPENHEARTH_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const override = env.OPENHEARTH_STATE_DIR?.trim() || env.OPENHEARTH_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        return path.join(homedir(), ".openhearth");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.OPENHEARTH_STATE_DIR = previous;
    }
