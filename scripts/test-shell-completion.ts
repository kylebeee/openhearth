 ...
 * `openhearth update`. Use it to verify the completion installation flow
...
\`openhearth update\`, \`openhearth doctor\`, and \`openhearth onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# OpenHearth Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`openhearth completion --install\` later to enable.`));
    return;
  }
