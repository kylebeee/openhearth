...
#
# Suggested cron: */30 * * * * /home/admin/openhearth/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send OpenHearth notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., openhearth-alerts)
...
STATE_FILE="$HOME/.openhearth/auth-monitor-state"
...
send_notification() {
    ...
    # Send via OpenHearth if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use openhearth
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via OpenHearth to $NOTIFY_PHONE..."
            openhearth send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! OpenHearth is down. Run: ssh l36 '~/openhearth/scripts/mobile-reauth.sh'" "urgent"
    exit 1
