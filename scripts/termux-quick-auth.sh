...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/openhearth/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "OpenHearth Re-Auth" -c "After getting key, run: ssh $SERVER '~/openhearth/scripts/mobile-reauth.sh'" --id openhearth-auth
        ;;
    *)
