...
#
# Env:
#   DMG_VOLUME_NAME        default: CFBundleName (or "OpenHearth")
...
APP_NAME=$(/usr/libexec/PlistBuddy -c "Print CFBundleName" "$APP_PATH/Contents/Info.plist" 2>/dev/null || echo "OpenHearth")
...
DMG_TEMP="$(mktemp -d /tmp/openhearth-dmg.XXXXXX)"
...
if [[ "${SKIP_DMG_STYLE:-0}" != "1" ]]; then
  ...
  ICON_SRC="$ROOT_DIR/apps/macos/Sources/OpenHearth/Resources/OpenHearth.icns"
  if [[ -f "$ICON_SRC" ]]; then
...
hdiutil resize -limits "$DMG_RW_PATH" >/tmp/openhearth-dmg-limits.txt 2>/dev/null || true
MIN_SECTORS="$(tail -n 1 /tmp/openhearth-dmg-limits.txt 2>/dev/null | awk '{print $1}')"
rm -f /tmp/openhearth-dmg-limits.txt
