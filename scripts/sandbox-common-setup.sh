...
BASE_IMAGE="${BASE_IMAGE:-openhearth-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-openhearth-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=openhearth.sandbox=1)
NOTE
