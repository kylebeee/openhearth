#!/bin/bash
# Setup OpenHearth Auth Management System
...
echo "=== OpenHearth Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'openhearth-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  OpenHearth message: Send warning via OpenHearth itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/openhearth-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/openhearth-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/openhearth-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now openhearth-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status openhearth-auth-monitor.timer"
echo ""
echo "Quick commands:"
