...
INSTALL_URL="${OPENHEARTH_INSTALL_URL:-https://openhearth.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${OPENHEARTH_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${OPENHEARTH_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="openhearth"
PACKAGE_NAME="${OPENHEARTH_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (OPENHEARTH_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${OPENHEARTH_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${OPENHEARTH_INSTALL_LATEST_OUT:-}"
fi
