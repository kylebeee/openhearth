...
const shouldBuild = (deps) => {
  if (deps.env.OPENHEARTH_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.OPENHEARTH_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[openhearth] ${message}\n`);
};

const runOpenHearth = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["openhearth.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
...
export async function runNodeMain(params = {}) {
  ...
  if (!shouldBuild(deps)) {
    return await runOpenHearth(deps);
  }
