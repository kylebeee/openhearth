...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/openhearth/openhearth/main/appcast.xml}"
AUTO_CHECKS=true
...
OPENCLAWKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/OpenHearthKit_OpenHearthKit.bundle"
if [ -d "$OPENCLAWKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/OpenHearthKit_OpenHearthKit.bundle"
  cp -R "$OPENCLAWKIT_BUNDLE" "$APP_ROOT/Contents/Resources/OpenHearthKit_OpenHearthKit.bundle"
else
  echo "WARN: OpenHearthKit resource bundle not found at $OPENCLAWKIT_BUNDLE (continuing)" >&2
fi
