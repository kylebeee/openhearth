...
IMAGE_NAME="openhearth-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="openhearth-net-e2e-$$"
GW_NAME="openhearth-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "OPENHEARTH_GATEWAY_TOKEN=$TOKEN" \
	  -e "OPENHEARTH_SKIP_CHANNELS=1" \
	  -e "OPENHEARTH_SKIP_GMAIL_WATCHER=1" \
	  -e "OPENHEARTH_SKIP_CRON=1" \
	  -e "OPENHEARTH_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
