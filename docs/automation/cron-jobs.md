...
- Jobs persist under `~/.openhearth/cron/` so restarts don’t lose schedules.
...
openhearth cron run <job-id>
openhearth cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.openhearth/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `openhearth cron add/edit` or the cron
...
If `delivery` is omitted for isolated jobs, OpenHearth defaults to `announce`.
...
- Job store: `~/.openhearth/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.openhearth/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
```json5
{
  cron: {
    ...
    store: "~/.openhearth/cron/jobs.json",
...
openhearth cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
openhearth cron edit <jobId> --agent ops
openhearth cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
openhearth cron run <jobId>
openhearth cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
openhearth cron edit <jobId> \
...
```bash
openhearth system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`openhearth system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `OPENHEARTH_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- OpenHearth applies exponential retry backoff for recurring jobs after consecutive errors:
