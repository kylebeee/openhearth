---
summary: "OpenHearth on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up OpenHearth on a Raspberry Pi
  - Running OpenHearth on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# OpenHearth on Raspberry Pi

## Goal

Run a persistent, always-on OpenHearth Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most OpenHearth features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled openhearth

# Enable if not
sudo systemctl enable openhearth

# Start on boot
sudo systemctl start openhearth
```
...
```bash
# Check logs
journalctl -u openhearth --no-pager -n 100

# Common fix: rebuild
cd ~/openhearth  # if using hackable install
npm run build
sudo systemctl restart openhearth
```
