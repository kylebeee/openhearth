...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/openhearth`)
...
If the page fails to load, start the Gateway first: `openhearth gateway`.
...
you revoke it with `openhearth devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.kylebeee/openhearth.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. OpenHearth
...
OpenHearth **blocks** Control UI connections without device identity.
