...
```bash
openhearth tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `openhearth tui --deliver`
...
- Check the Gateway logs: `openhearth logs --follow`.
- Confirm the agent can run: `openhearth status` and `openhearth models status`.
...
- No agents in picker: check `openhearth agents list` and your routing config.
