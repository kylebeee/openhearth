...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects OpenHearth to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
openhearth plugins install @openhearth/feishu
```

Local checkout (when running from a git repo):

```bash
openhearth plugins install ./extensions/feishu
```
...
If you just installed OpenHearth, run the wizard:
...
2. Configuring app credentials in OpenHearth
...
1. You already ran `openhearth channels add` for Feishu
2. The gateway is running (`openhearth gateway status`)
...
```bash
openhearth pairing approve feishu <CODE>
```
...
- **Approve pairing**:
  ...
  ```bash
  openhearth pairing list feishu
  openhearth pairing approve feishu <CODE>
  ```
...
2. Run `openhearth logs --follow` and look for `chat_id`
...
2. Run `openhearth logs --follow` and look for `open_id`
...
| Command                      | Description                   |
| ---------------------------- | ----------------------------- |
| `openhearth gateway status`  | Show gateway status           |
| `openhearth gateway install` | Install/start gateway service |
| `openhearth gateway stop`    | Stop gateway service          |
| `openhearth gateway restart` | Restart gateway service       |
| `openhearth logs --follow`   | Tail gateway logs             |
...
4. Check logs: `openhearth logs --follow`
...
5. Ensure the gateway is running: `openhearth gateway status`
6. Check logs: `openhearth logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.openhearth/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.openhearth/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
