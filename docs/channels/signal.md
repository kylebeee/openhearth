...
- OpenHearth installed on your server (Linux flow below tested on Ubuntu 24).
...
3. Choose one setup path:
   - **Path A (QR link):** `signal-cli link -n "OpenHearth"` and scan with Signal.
   - **Path B (SMS register):** register a dedicated number with captcha + SMS verification.
4. Configure OpenHearth and restart the gateway.
5. Send a first DM and approve pairing (`openhearth pairing approve signal <CODE>`).
...
4. Configure OpenHearth, restart gateway, verify channel:
...
5. Pair your DM sender:
   ...
   - Approve code on the server: `openhearth pairing approve signal <PAIRING_CODE>`.
...
- Config validation errors after edits: run `openhearth doctor --fix`.
...
openhearth pairing list signal
pgrep -af signal-cli
grep -i "signal" "/tmp/kylebeee/openhearth-$(date +%Y-%m-%d).log" | tail -20
```
