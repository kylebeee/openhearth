...
```bash
openhearth plugins install @openhearth/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
openhearth plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "OpenHearth" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `openhearth pairing list nextcloud-talk`
  - `openhearth pairing approve nextcloud-talk <CODE>`
