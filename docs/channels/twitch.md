...
read_when:
  - Setting up Twitch chat integration for OpenHearth
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. OpenHearth connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
openhearth plugins install @openhearth/twitch
```

Local checkout (when running from a git repo):

```bash
openhearth plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `OPENHEARTH_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "openhearth", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use OPENHEARTH_TWITCH_ACCESS_TOKEN env var)
...
```bash
OPENHEARTH_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
