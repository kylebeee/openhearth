...
```bash
openhearth plugins install @openhearth/msteams
```

Local checkout (when running from a git repo):

```bash
openhearth plugins install ./extensions/msteams
```
...
3. Configure OpenHearth with those credentials.
...
- Talk to OpenHearth via Teams DMs, group chats, or channels.
...
- On startup, OpenHearth resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.kylebeee/openhearth.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring OpenHearth, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                      |
   | ------------------ | ---------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `openhearth-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `openhearth plugins install @openhearth/msteams`
   - From a local checkout: `openhearth plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "OpenHearth in Teams", "full": "OpenHearth in Teams" },
...
OpenHearth handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, OpenHearth only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/OpenHearthShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

OpenHearth sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `openhearth message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.openhearth/msteams-polls.json`.
...
openhearth message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
openhearth message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
openhearth message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
openhearth message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
