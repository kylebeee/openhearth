---
summary: "Chrome extension: let OpenHearth drive your existing Chrome tab"
read_when:
...
The OpenHearth Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate openhearth-managed Chrome profile.
...
OpenHearth then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the OpenHearth release (npm package) as static files. There is no separate “build” step.

After upgrading OpenHearth:

- Re-run `openhearth browser extension install` to refresh the installed files under your OpenHearth state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

OpenHearth ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want OpenHearth to control.
...
- `ON`: attached; OpenHearth can drive that tab.
...
Debugging: `openhearth sandbox explain`
...
`openhearth browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `openhearth browser extension install` first to copy the extension to a stable location under your OpenHearth state directory.
...
- **This is not isolated** like the dedicated openhearth-managed profile.
