---
summary: "Agent tool surface for OpenHearth (browser, canvas, nodes, message, cron) replacing legacy `openhearth-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `openhearth-*` skills
title: "Tools"
---

# Tools (OpenHearth)

OpenHearth exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `openhearth-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `openhearth.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, OpenHearth logs a warning and ignores the allowlist so core tools stay available.
...
- `group:nodes`: `nodes`
- `group:openhearth`: all built-in OpenHearth tools (excludes provider plugins)
...
- `host=node` can target a macOS companion app or a headless node host (`openhearth node run`).
...
- Requires a Brave API key (recommended: `openhearth configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated OpenHearth-managed browser.
...
- Quick smoke: `openhearth nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `openhearth gateway` restart in-place)
