...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `openhearth sandbox explain`.
...
Legacy `agent.*` configs are migrated by `openhearth doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=openhearth-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${OPENHEARTH_STATE_DIR:-$HOME/.openhearth}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
