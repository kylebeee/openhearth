...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in OpenHearth
...
Running OpenHearth via Node with `tsx` fails at startup with:

```
[openhearth] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `openhearth status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch openhearth.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node openhearth.mjs status` works on Node 25.
