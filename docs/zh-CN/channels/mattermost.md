...
read_when:
  - 设置 Mattermost
  - 调试 Mattermost 路由
summary: Mattermost 机器人设置和 OpenHearth 配置
title: Mattermost
x-i18n:
...
```bash
openhearth plugins install @openhearth/mattermost
```

本地检出（从 git 仓库运行时）：

```bash
openhearth plugins install ./extensions/mattermost
```

如果你在配置/新手引导期间选择 Mattermost 并检测到 git 检出，OpenHearth 会自动提供本地安装路径。
...
4. 配置 OpenHearth 并启动 Gateway 网关。
...
- 通过以下方式批准：
  - `openhearth pairing list mattermost`
  - `openhearth pairing approve mattermost <CODE>`
...
在 `openhearth message send` 或 cron/webhooks 中使用这些目标格式：
