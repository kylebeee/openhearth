...
LINE 通过 LINE Messaging API 连接到 OpenHearth。该插件作为 webhook 接收器在 Gateway 网关上运行，使用你的 channel access token + channel secret 进行身份验证。
...
```bash
openhearth plugins install @openhearth/line
```

本地检出（从 git 仓库运行时）：

```bash
openhearth plugins install ./extensions/line
```
...
```bash
openhearth pairing list line
openhearth pairing approve line <CODE>
```
