...
summary: 设置指南：在保持最新的同时保持你的 OpenHearth 设置个性化
title: 设置
x-i18n:
...
- **个性化设置存放在仓库之外：** `~/.openhearth/workspace`（工作区）+ `~/.kylebeee/openhearth.json`（配置）。
...
- **配置：** `~/.kylebeee/openhearth.json`（JSON/JSON5 格式）
- **工作区：** `~/.openhearth/workspace`（Skills、提示、记忆；将其设为私有 git 仓库）
...
如果你还没有全局安装，通过 `pnpm openhearth setup` 运行它。

## 稳定工作流（macOS 应用优先）

1. 安装并启动 **OpenHearth.app**（菜单栏）。
...
- 运行 `openhearth setup`，然后 `openhearth channels login`，然后手动启动 Gateway 网关（`openhearth gateway`）。
...
- **状态存储位置：**
  ...
  - 会话：`~/.openhearth/agents/<agentId>/sessions/`
  - 日志：`/tmp/openhearth/`
...
- **配对允许列表**：`~/.openhearth/credentials/<channel>-allowFrom.json`
- **模型认证配置文件**：`~/.openhearth/agents/<agentId>/agent/auth-profiles.json`
- **旧版 OAuth 导入**：`~/.openhearth/credentials/oauth.json`
  更多详情：[安全](/gateway/security#credential-storage-map)。

## 更新（不破坏你的设置）

- 将 `~/.openhearth/workspace` 和 `~/.openhearth/` 保持为"你的东西"；不要将个人提示/配置放入 `openhearth` 仓库。
...
- [OpenHearth 助手设置](/start/openhearth)
