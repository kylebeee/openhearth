...
你可以在 `~/.kylebeee/openhearth.json` 中覆盖此设置：

```json
{
  "logging": {
    "file": "/path/to/openhearth.log"
  }
}
```
...
```bash
openhearth channels logs --channel whatsapp
```
...
所有日志配置都在 `~/.kylebeee/openhearth.json` 的 `logging` 下。

```json
{
  "logging": {
    "level": "info",
    "file": "/tmp/kylebeee/openhearth-YYYY-MM-DD.log",
    "consoleLevel": "info",
    "consoleStyle": "pretty",
...
- OpenHearth 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
```
OPENHEARTH_DIAGNOSTICS=telegram.http,telegram.payload
```
...
```json
{
  ...
  "diagnostics": {
    ...
    "otel": {
      ...
      "serviceName": "openhearth-gateway",
      "traces": true,
      "metrics": true,
      "logs": true,
      "sampleRate": 0.2,
...
- 你也可以使用 `openhearth plugins enable diagnostics-otel` 启用插件。
...
- `openhearth.tokens`（计数器，属性：`openhearth.token`、`openhearth.channel`、`openhearth.provider`、`openhearth.model`）
- `openhearth.cost.usd`（计数器，属性：`openhearth.channel`、`openhearth.provider`、`openhearth.model`）
- `openhearth.run.duration_ms`（直方图，属性：`openhearth.channel`、`openhearth.provider`、`openhearth.model`）
- `openhearth.context.tokens`（直方图，属性：`openhearth.context`、`openhearth.channel`、`openhearth.provider`、`openhearth.model`）

消息流：

- `openhearth.webhook.received`（计数器，属性：`openhearth.channel`、`openhearth.webhook`）
- `openhearth.webhook.error`（计数器，属性：`openhearth.channel`、`openhearth.webhook`）
- `openhearth.webhook.duration_ms`（直方图，属性：`openhearth.channel`、`openhearth.webhook`）
- `openhearth.message.queued`（计数器，属性：`openhearth.channel`、`openhearth.source`）
- `openhearth.message.processed`（计数器，属性：`openhearth.channel`、`openhearth.outcome`）
- `openhearth.message.duration_ms`（直方图，属性：`openhearth.channel`、`openhearth.outcome`）

队列 + 会话：

- `openhearth.queue.lane.enqueue`（计数器，属性：`openhearth.lane`）
- `openhearth.queue.lane.dequeue`（计数器，属性：`openhearth.lane`）
- `openhearth.queue.depth`（直方图，属性：`openhearth.lane` 或 `openhearth.channel=heartbeat`）
- `openhearth.queue.wait_ms`（直方图，属性：`openhearth.lane`）
- `openhearth.session.state`（计数器，属性：`openhearth.state`、`openhearth.reason`）
- `openhearth.session.stuck`（计数器，属性：`openhearth.state`）
- `openhearth.session.stuck_age_ms`（直方图，属性：`openhearth.state`）
- `openhearth.run.attempt`（计数器，属性：`openhearth.attempt`）

### 导出的 span（名称 + 关键属性）

- `openhearth.model.usage`
  - `openhearth.channel`、`openhearth.provider`、`openhearth.model`
  - `openhearth.sessionKey`、`openhearth.sessionId`
  - `openhearth.tokens.*`（input/output/cache_read/cache_write/total）
- `openhearth.webhook.processed`
  - `openhearth.channel`、`openhearth.webhook`、`openhearth.chatId`
- `openhearth.webhook.error`
  - `openhearth.channel`、`openhearth.webhook`、`openhearth.chatId`、`openhearth.error`
- `openhearth.message.processed`
  - `openhearth.channel`、`openhearth.outcome`、`openhearth.chatId`、`openhearth.messageId`、`openhearth.sessionKey`、`openhearth.sessionId`、`openhearth.reason`
- `openhearth.session.stuck`
  - `openhearth.state`、`openhearth.ageMs`、`openhearth.queueDepth`、`openhearth.sessionKey`、`openhearth.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `openhearth doctor`。
