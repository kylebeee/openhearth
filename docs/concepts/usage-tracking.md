...
- `/usage cost` in chats: local cost summary aggregated from OpenHearth session logs.
- CLI: `openhearth status --usage` prints a full per-provider breakdown.
- CLI: `openhearth channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
