...
read_when:
  - You want to understand what “context” means in OpenHearth
...
“Context” is **everything OpenHearth sends to the model for a run**. It is bounded by the model’s **context window** (token limit).

Beginner mental model:

- **System prompt** (OpenHearth-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How OpenHearth builds the system prompt

The system prompt is **OpenHearth-owned** and rebuilt each run. It includes:
...
By default, OpenHearth injects a fixed set of workspace files (if present):
