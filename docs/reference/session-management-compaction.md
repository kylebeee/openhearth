...
This document explains how OpenHearth manages sessions end-to-end:
...
OpenHearth is designed around a single **Gateway process** that owns session state.
...
OpenHearth persists sessions in two layers:
...
- Transcripts: `~/.openhearth/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

OpenHearth resolves these via `src/config/sessions.ts`.
...
OpenHearth intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (OpenHearth consumes the events, but Pi decides when to compact).
...
OpenHearth also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, OpenHearth bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, OpenHearth leaves it alone.
...
- `openhearth status` (CLI)
- `openhearth sessions` / `sessions --json`
...
OpenHearth supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- OpenHearth strips/suppresses this in the delivery layer.

As of `2026.1.10`, OpenHearth also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
OpenHearth uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but OpenHearth’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `openhearth status`.
