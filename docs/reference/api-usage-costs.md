...
OpenHearth features that can generate provider usage or paid API calls.
...
- `openhearth status --usage` and `openhearth channels list` show provider **usage windows**
...
OpenHearth can pick up credentials from:
...
- `openhearth status --usage`
- `openhearth models status --json`
...
`openhearth models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
