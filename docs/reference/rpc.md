...
OpenHearth integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- OpenHearth owns lifecycle when `channels.signal.autoStart=true`.
...
- OpenHearth spawns `imsg rpc` as a child process (legacy iMessage integration).
