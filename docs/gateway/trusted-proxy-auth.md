...
read_when:
  - Running OpenHearth behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of OpenHearth
...
- You run OpenHearth behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. OpenHearth checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. OpenHearth extracts the user identity from the configured header
...
```yaml
routes:
  - from: https://openhearth.example.com
    to: http://openhearth-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
```
openhearth.example.com {
    authenticate with oauth2_provider
    authorize with policy1

    reverse_proxy openhearth:18789 {
...
location / {
    ...
    proxy_pass http://openhearth:18789;
...
`openhearth security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update OpenHearth config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `openhearth security audit` and review findings
