...
- Use the macOS app’s **Remote over SSH** mode (Settings → General → “OpenHearth runs”).
...
- `openhearth health` and `openhearth status --deep` now reach the remote gateway via `ws://127.0.0.1:18789`.
- `openhearth gateway {status,health,send,agent,call}` can also target the forwarded URL via `--url` when needed.

Note: replace `18789` with your configured `gateway.port` (or `--port`/`OPENHEARTH_GATEWAY_PORT`).
