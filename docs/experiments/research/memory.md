...
read_when:
  - Designing workspace memory (~/.openhearth/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep OpenHearth integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.openhearth/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.openhearth/workspace` as canonical human-readable memory.
...
```
~/.openhearth/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in OpenHearth**, but keep a separable core library.

### Why integrate into OpenHearth?

- OpenHearth already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `openhearth memory recall "…" --k 25 --since 30d`
  - `openhearth memory reflect --since 7d`
...
Pragmatic take for `~/.openhearth/workspace`:
