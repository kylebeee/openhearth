...
- Publish as `openhearth/plugin-sdk` (or export from core under `openhearth/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `OpenHearthPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: OpenHearthConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: OpenHearthConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: OpenHearthConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: OpenHearthConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: OpenHearthConfig): string;
  };
};
```
...
- Introduce `openhearth/plugin-sdk`.
- Add `api.runtime` to `OpenHearthPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `openhearthRuntime: ">=2026.2.0"`).
