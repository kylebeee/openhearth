---
summary: "Use Venice AI privacy-focused models in OpenHearth"
read_when:
  - You want privacy-focused inference in OpenHearth
...
```bash
openhearth onboard --auth-choice venice-api-key
```
...
```bash
openhearth chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, OpenHearth shows all available Venice models. Pick based on your needs:
...
```bash
openhearth models set venice/claude-opus-45
openhearth models set venice/llama-3.3-70b
```

List all available models:

```bash
openhearth models list | grep venice
```

## Configure via `openhearth configure`

1. Run `openhearth configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
OpenHearth automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
openhearth chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
openhearth chat --model venice/claude-opus-45

# Use uncensored model
openhearth chat --model venice/venice-uncensored

# Use vision model with image
openhearth chat --model venice/qwen3-vl-235b-a22b

# Use coding model
openhearth chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `openhearth models list` to see currently available models. Some models may be temporarily offline.
