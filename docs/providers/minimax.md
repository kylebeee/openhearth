---
summary: "Use MiniMax M2.1 in OpenHearth"
read_when:
  - You want MiniMax models in OpenHearth
...
```bash
openhearth plugins enable minimax-portal-auth  # skip if already loaded.
openhearth gateway restart  # restart if gateway is already running
openhearth onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/kylebeee/openhearth/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `openhearth.json`:
...
- Use `openhearth models list` and `openhearth models set minimax/MiniMax-M2.1` to switch.
...
- Running `openhearth configure` and selecting **MiniMax M2.1**, or
