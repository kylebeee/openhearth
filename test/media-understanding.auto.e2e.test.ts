...
import type { OpenHearthConfig } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("openhearth-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("openhearth-bin-sherpa-");
      const modelDir = await makeTempDir("openhearth-sherpa-model-");
      ...
      const cfg: OpenHearthConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("openhearth-bin-whispercpp-");
      const modelDir = await makeTempDir("openhearth-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("openhearth-bin-gemini-");
      ...
      const cfg: OpenHearthConfig = { tools: { media: { image: {} } } };
